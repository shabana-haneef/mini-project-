...
import {
    LayoutDashboard, Upload, FileUp, Library, BarChart3, LogOut,
...
const MemberNavbar = () => {
    ...
    const baseNavLinks = [
        ...
        { name: 'Upload Resources', path: '/mentor/upload-resource', icon: FileUp, operational: true },
        { name: 'My Uploaded Content', path: '/mentor/resources', icon: Library },
        { name: 'Analytics', path: '/mentor/analytics', icon: BarChart3 },
    ];

    const navLinks = baseNavLinks.filter(link => !(link.operational && user?.role === 'superadmin'));
        ...
        <header className="sticky top-0 z-40 w-full glass-morphism border-b border-white/10 px-4 sm:px-6 h-16 flex items-center justify-between transition-all duration-300">
            <div className="flex items-center space-x-8">
                <div className="flex items-center space-x-2">
                    ...
                    <span className="text-xl font-bold tracking-tight text-white hidden sm:block">Campus<span className="text-accent-secondary">Academy</span></span>
                </div>
            ...
            <div className="flex items-center space-x-2 sm:space-x-4">
                ...
                {user && (
                    <div className="relative">
                        ...
                        >
                            <div className="w-8 h-8 rounded-full bg-linear-to-tr from-accent-secondary to-purple-500 flex items-center justify-center text-white font-bold text-sm shadow-inner">
                                {user?.name?.charAt(0) || 'M'}
                            </div>
                            <div className="hidden md:flex flex-col items-start text-left">
                                <span className="text-sm font-bold text-white leading-none mb-1">{user?.name || 'Mentor'}</span>
                                <span className={`text-[10px] font-black uppercase tracking-widest leading-none ${user?.role === 'superadmin' ? 'text-red-500' : 'text-accent-secondary'}`}>
                                    {user?.role === 'superadmin' ? 'Super Admin' : 'Mentor Portal'}
                                </span>
                            </div>
                        </button>
                        ...
                        {showProfileMenu && (
                            <div className="absolute right-0 mt-2 w-56 glass-morphism border border-white/10 rounded-2xl shadow-2xl py-2 z-50 animate-in fade-in slide-in-from-top-2">
                                <Link to="/mentor/settings" className="flex items-center px-4 py-3 text-sm text-slate-300 hover:bg-white/5 hover:text-white transition-colors" onClick={() => setShowProfileMenu(false)}>
